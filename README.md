...
8. Com a configuração de filtro feita, as próximas alterações serão nas abas Partition, que vai receber apenas uma configuração que é "all" em "segment granularity" como marcado na primeira imagem da figura abaixo. Depois vem a aba Publish, que recebe 2 configurações, a primeira é o nome da base que é configurado em "Datasource name" e a segunda é se for adicionado mais arquivos a essa mesma configuração no druid ele concatena com os existentes, essa config recebe False. Ambas as configurações estão marcadas na figura abaixo.
![](https://github.com/Antonio-Borges-Rufino/Dados_Aeronauticos_Data_Pipeline/blob/main/imgs/druid_10.jpg)
9. Após essas configurações, aperte em seguir até você ser levado a aba ingestion. Nesse caso, não aparecerá a tabela como ingestion, como a inserção é em batch aparece só o nome do processo feito na parte Task Id. Para visualizar a tabela inserida vá até a aba Datasource que deverá ter a tabela funcionando lá. A imagem abaixo mostra como deve ser apresentada a tabela caso tudo ocorra bem.
